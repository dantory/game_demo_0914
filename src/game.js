...
export class Courtyard {
  constructor(canvas, manifest, images, onState) {
    ...
    this.reset();
    this.keydown=e=>{const key=e.key.toLowerCase();if(['w','a','s','d'].includes(key)){e.preventDefault();this.keys.add(key);}if(key==='q'){e.preventDefault();this.useSkill('cleave');}if(e.code==='Space'||key===' '){e.preventDefault();this.useSkill('dodge');}};
  ...
  reset(){
    ...
    this.player={x:720,y:520,r:11,hp:maxHp,maxHp,face:'down',action:'idle',clock:0,cool:0,inv:0,dodge:null,aimX:720,aimY:620};this.camera={x:240,y:250};
    ...
    this.random??=Math.random;this.lootSerial=0;this.keys.clear();this.touchMove={x:0,y:0};this.particles=[];this.projectiles=[];this.drops=[];this.texts=[];this.time=0;this.hitStop=0;this.shake=0;this.attackQueued=false;this.attackHeld=false;this.mobileAutoAim=false;this.skillQueued=null;this.skillCooldowns={cleave:0,dodge:0};this.spawnWave();
  }
  spawnWave(){
  ...
  movementInput(){let x=Number(this.keys.has('d'))-Number(this.keys.has('a'))+this.touchMove.x,y=Number(this.keys.has('s'))-Number(this.keys.has('w'))+this.touchMove.y;const length=Math.hypot(x,y);if(length>1){x/=length;y/=length;}return{x,y};}
  ...
  performDodge(){
    if(this.skillCooldowns.dodge>0||this.player.dodge)return false;const p=this.player,input=this.movementInput(),faces={up:{x:0,y:-1},down:{x:0,y:1},left:{x:-1,y:0},right:{x:1,y:0}},direction=input.x||input.y?input:faces[p.face]||faces.down;
    this.skillCooldowns.dodge=2.4;p.dodge={x:direction.x,y:direction.y,time:.2,duration:.2};p.face=Math.abs(direction.x)>Math.abs(direction.y)?direction.x>0?'right':'left':direction.y>0?'down':'up';p.inv=.3;p.action='walk';p.clock=0;p.cool=.24;this.attackQueued=false;this.burst(p.x,p.y,'#8dc7dc',14,85);return true;
  }
  ...
  update(dt){
    ...
    if(this.state!=='playing')return;if(this.waveDelay>0){this.waveDelay-=dt;if(this.waveDelay<=0){this.wave++;this.spawnWave();}return;}if(this.skillQueued){const skill=this.skillQueued;this.skillQueued=null;if(skill==='cleave')this.performCleave();if(skill==='dodge')this.performDodge();}
    p.cool=Math.max(0,p.cool-dt);p.inv=Math.max(0,p.inv-dt);p.clock+=dt;if(this.mobileAutoAim)this.aimNearestEnemy();const input=this.movementInput(),dx=input.x,dy=input.y;
    if(p.dodge){const slice=Math.min(dt,p.dodge.time),speed=660*(1+this.gear.speed/200),beforeX=p.x,beforeY=p.y;this.move(p,p.dodge.x*speed*slice,p.dodge.y*speed*slice);p.dodge.time-=slice;p.inv=Math.max(p.inv,.08);if(Math.hypot(p.x-beforeX,p.y-beforeY)<1||p.dodge.time<=0){p.dodge=null;p.action='idle';p.clock=0;this.burst(p.x,p.y,'#b8e5ef',8,48);}}
    if(p.action==='attack'&&p.clock>(p.skill==='cleave'?.46:.32)){p.action='idle';p.skill=null;p.clock=0;}
    if(!p.dodge&&p.action!=='attack'){
  ...
  draw(){
    ...
    const aimAngle=Math.atan2(this.player.aimY-this.player.y,this.player.aimX-this.player.x);if(this.player.action==='attack'&&this.player.clock<.4){const cleave=this.player.skill==='cleave';c.strokeStyle=cleave?'#efad52':'#f5d98c';c.lineWidth=cleave?7:4;c.beginPath();c.arc(this.player.x,this.player.y-6,cleave?92:47,cleave?0:aimAngle-.8,cleave?Math.PI*2:aimAngle+.8);c.stroke();}if(this.player.dodge){c.strokeStyle='#a7e6f0bb';c.lineWidth=3;c.beginPath();c.arc(this.player.x,this.player.y-7,24+Math.sin(this.time*28)*3,0,Math.PI*2);c.stroke();c.strokeStyle='#d8f7ff88';c.beginPath();c.moveTo(this.player.x-this.player.dodge.x*48,this.player.y-this.player.dodge.y*48-7);c.lineTo(this.player.x-this.player.dodge.x*15,this.player.y-this.player.dodge.y*15-7);c.stroke();}
