...
export class Courtyard {
  ...
  draw(){
    ...
    for(const drop of this.drops){const bob=drop.y-8-Math.sin(this.time*9)*3;c.globalAlpha=Math.min(1,drop.life*2);if(drop.type==='item'){const color=drop.item.rarity==='legendary'?'#e8793e':drop.item.rarity==='rare'?'#e3c151':drop.item.rarity==='magic'?'#5aa8e8':'#b8b7ae',icon=this.images[`item-${itemIconKey(drop.item)}`];c.fillStyle=color+'55';c.fillRect(drop.x-3,drop.y-57,6,46);c.fillStyle='#080c12dd';c.fillRect(drop.x-18,bob-18,36,36);c.strokeStyle=color;c.lineWidth=2;c.strokeRect(drop.x-18.5,bob-18.5,37,37);if(icon)c.drawImage(icon,drop.x-16,bob-16,32,32);}else{c.fillStyle=drop.type==='ruby'?'#d8495c':drop.type==='potion'?'#b9273f':'#e7c45f';c.fillRect(drop.x-5,bob,10,10);c.fillStyle=drop.type==='potion'?'#ff8b8b':'#fff2aa';c.fillRect(drop.x-2,bob+1,3,3);}}c.globalAlpha=1;
