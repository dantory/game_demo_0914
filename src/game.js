...
export class Courtyard {
  ...
  spawnWave(){
    const spots=[[430,250],[555,235],[735,250],[435,410],[590,430],[770,410],[835,310],[690,145],[390,370],[820,170],[470,455],[860,435],[620,165],[330,430],[790,120],[880,270],[535,465],[410,115]],count=7+this.wave*3;
  ...
  gainReward(enemy){
    this.kills++;this.waveKills++;this.combo++;this.comboClock=2.2;this.gold+=enemy.type==='brute'?24:3+this.wave;this.xp+=enemy.type==='brute'?8:2;this.drops.push({x:enemy.x,y:enemy.y,life:3,type:enemy.type==='brute'?'ruby':this.kills%4===0?'potion':'coin'});
  ...
  update(dt){
    ...
    this.drops=this.drops.filter(a=>{a.life-=dt;const d=Math.hypot(a.x-p.x,a.y-p.y);if(d<52){a.x+=(p.x-a.x)*dt*7;a.y+=(p.y-a.y)*dt*7;}if(d<18){if(a.type==='potion')p.hp=Math.min(p.maxHp,p.hp+12);this.gold+=a.type==='coin'?1:0;this.burst(a.x,a.y,a.type==='potion'?'#d95b63':'#e6c15d',5,35);return false;}return a.life>0;});
  ...
  hurt(amount){const p=this.player;p.hp=Math.max(0,p.hp-amount);p.inv=1;p.action='idle';this.combo=0;this.shake=9;this.hitStop=.045;this.floatText(p.x,p.y-34,amount,'#ff6a62');this.burst(p.x,p.y,'#dc5e58',18,105);if(p.hp<=0)this.state='lost';}
  ...
  draw(){
    const c=this.ctx,map=this.images.map;c.save();const sx=this.shake?Math.round((Math.random()-.5)*this.shake):0,sy=this.shake?Math.round((Math.random()-.5)*this.shake):0;c.translate(sx,sy);c.imageSmoothingEnabled=false;if(map)c.drawImage(map,0,0,960,540);else{c.fillStyle='#0d1119';c.fillRect(0,0,960,540);}
    c.globalAlpha=.14;c.fillStyle='#86a872';for(let y=88;y<486;y+=32)for(let x=72+(y/32%2)*16;x<896;x+=32)if(!this.blocked(x,y,3)&&((x+y)%5<2))c.fillRect(x,y,3,3);c.globalAlpha=1;
    ...
    for(const drop of this.drops){const bob=drop.y-8-Math.sin(this.time*9)*3;c.globalAlpha=Math.min(1,drop.life*2);c.fillStyle=drop.type==='ruby'?'#d8495c':drop.type==='potion'?'#b9273f':'#e7c45f';c.fillRect(drop.x-5,bob,10,10);c.fillStyle=drop.type==='potion'?'#ff8b8b':'#fff2aa';c.fillRect(drop.x-2,bob+1,3,3);}c.globalAlpha=1;
    ...
    if(this.waveDelay>0){c.fillStyle='#090d13dd';c.fillRect(336,232,288,68);c.strokeStyle='#9d7945';c.strokeRect(336.5,232.5,287,67);c.fillStyle='#dfc274';c.font='700 15px Georgia';c.textAlign='center';c.fillText(`WAVE ${this.wave+1} APPROACHES`,480,272);}
