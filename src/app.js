...
async function boot(){
 try{
  const response=await fetch('assets/manifest.json?v=2');if(!response.ok)throw Error('asset manifest');const manifest=await response.json(),images={};
...
canvas.addEventListener('pointermove',e=>{if(game){const p=point(e);game.setAim(p.x,p.y);}});canvas.addEventListener('pointerdown',e=>{if(!game||game.paused||e.button!==0)return;e.preventDefault();const p=point(e);game.setAim(p.x,p.y);game.queueAttack();canvas.focus();});canvas.addEventListener('contextmenu',e=>e.preventDefault());
...
document.querySelector('[data-action="attack"]').onpointerdown=e=>{e.preventDefault();if(game&&!game.paused)game.queueAttack();};document.addEventListener('visibilitychange',()=>{if(document.hidden&&game&&game.state==='playing'){game.paused=true;game.keys.clear();show('전투 일시정지','준비되면 계속 플레이하세요.','계속하기');}});boot();
