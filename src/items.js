...
export function generateItem({wave=1,boss=false,rng=Math.random,id}={}){
  const slot=['weapon','armor','charm'][Math.floor(rng()*3)],rarity=rarityFor(wave,boss,rng),power=RARITIES[rarity].power,level=Math.max(1,wave),roll=()=>.85+rng()*.3,stats={};
...
export function loadInventory(storage=globalThis.localStorage){try{const data=JSON.parse(storage?.getItem('ashen-siege-inventory-v1'));if(data?.version===1&&Array.isArray(data.items)&&data.equipment)return data;}catch{}return createInventory();}
