...
import {addItem,bonuses,createInventory,discardItem,equipItem,generateItem,itemIconKey,ITEM_ICON_PATHS,loadInventory,saveInventory} from '../src/items.js';

const sequence=(...values)=>{let i=0;return()=>values[i++%values.length];};
test('generated loot has a valid slot, rarity, icon and combat stats',()=>{const item=generateItem({wave:2,rng:sequence(.1,.9,.5,.2,.1,.3),id:'one'});assert.ok(['weapon','armor','charm'].includes(item.slot));assert.ok(item.name);assert.ok(ITEM_ICON_PATHS[itemIconKey(item)]);assert.ok(Object.values(item.stats).some(x=>x>0));});
