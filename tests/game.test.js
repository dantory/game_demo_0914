...
test('clearing a wave advances to the next assault',()=>{const g=game();g.enemies=[enemy(190,286,1)];g.waveTarget=1;g.setAim(300,286);g.queueAttack();g.update(.016);assert.equal(g.waveKills,1);assert.ok(g.waveDelay>0);g.update(1.2);assert.equal(g.wave,2);assert.equal(g.enemies.length,8);});
